...
Attaccante::Attaccante() : velocita(50), tiro(50), passaggio(50), dribbling(50), difesa(50), fisico(50) {}
Attaccante::Attaccante(std::string n, std::string cgn, int e, std::string cam, std::string cl, std::string naz) :
    velocita(50), tiro(50), passaggio(50), dribbling(50), difesa(50), fisico(50), Giocatore(n,cgn,e,cam,cl,naz) {}

Attaccante::~Attaccante() {}

double Attaccante::calcolaOverallGiocatore() const{
    return ceil((difesa*0.02) + (fisico*0.05) + (velocita*0.30) + (tiro*0.30) + (passaggio*0.08) + (dribbling*0.25));
}

void Attaccante::SetStats(int vel, int tir, int pass, int dribb, int dif, int fis){
    velocita=vel;
    tiro=tir;
    passaggio=pass;
    dribbling=dribb;
...
std::string Attaccante::getAllInfo() const{
    std::string s;
    s.append("\n");
